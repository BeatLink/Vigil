...
from vigil.plugins.ports import PortsPlugin, _parse_results, _safe_metric_name, _build_probe_script
...
def plugin(make_plugin):
    return make_plugin(PortsPlugin, BASE_CFG)
...
class TestPortsCollection:
    ...
    async def test_no_checks_offline(self, make_plugin):
        p = make_plugin(PortsPlugin, {**BASE_CFG, "checks": []})
        await p.on_collect()
